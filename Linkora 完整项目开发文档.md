...
```txt
Linkora 先作为免费开源、自托管优先的短链系统推进。
近期不做收费 SaaS，不做多租户商业托管，不把功能设计绑定到付费墙。
下一阶段重点是把项目整理成人人可以部署的开源方案，让陌生用户 fork / clone 后能按文档和模板顺利部署到自己的 Cloudflare 账号。
未来如果商业化，优先考虑部署服务、迁移服务、托管服务或企业支持，但不得牺牲免费自托管版本的完整可用性。
```
...
9. 免费开源和自托管体验优先，商业化功能不得破坏个人用户完整使用。
10. 面向外部用户时，部署文档、模板配置和首次使用体验必须足够清晰。
```
...
环境变量和 Secrets 清单
首次部署 smoke test 脚本
从 Shlink / Sink / YOURLS / Dub 迁移的公开教程
去除个人域名和个人项目名硬编码
可选一键部署或半自动部署脚本
```
...
V5 以后优先打磨开源发布、部署模板、文档和上手体验。
除非明确决定商业化，否则不要引入收费墙、订阅计费、多租户 SaaS 复杂度。
```
